...
| TiebaLite parity | Current `main` and public `v0.65.0-alpha.8`: about 81% overall (estimated range 80–82%, with 18–20% remaining). Anonymous reading and media remain about 91–95% |
| Distribution | The public SideStore/LiveContainer source currently serves `v0.65.0-alpha.8` (build 86) |
...
- **`v0.65.0-alpha.2` native profile text editing:** An active complete account can
  ...
  and Web session probes must agree on the expected account UID, after which
  Core reads a fresh V12 preflight snapshot. The wire-level `intro` remains the
  edit source; presentation prefers a nonempty `display_intro` and falls back to
  `intro` only for legacy responses. It also
  freezes Tieba's existing birthday timestamp and visibility bit even though
  this version does not expose a birthday control; a missing or malformed
  birthday snapshot disables saving instead of submitting defaults. Nicknames
  under review likewise disable another mutation while retaining a readable
  profile summary. A changed save sends one signed HTTPS
  `/c/c/profile/modify` request at most, never automatically retries, and always
  performs one same-credential profile readback. Success requires the requested
  nickname to be active or explicitly pending review, the editable biography
  and sex to match, and the frozen birthday to remain unchanged. Equivalent
...
- **`v0.65.0-alpha.2` primary navigation:** The app now exposes Home, Explore,
  Messages, and My as four ordered system tabs by default, matching TiebaLite's
  reachable primary destinations. An independent default-on setting can hide
  Explore, leaving Home, Messages, and My in order. It also hides Home's redundant
  Explore shortcut; hot topics and explicit link pasting remain controlled by the
  separate Home discovery-section preference. If Explore is active when hidden,
  selection moves to Home without discarding Explore's private root path; the
  hidden root is not constructed and cannot start requests. Re-enabling the
  setting retains that path. With Explore hidden, personalized-discovery and hot-
  thread startup choices open the corresponding Explore destination in Home's
  stack, while hot topics opens directly there; the saved startup choice and the
  independent Home discovery-section preference are not rewritten. Each visible
  tab owns an independent `NavigationStack`, so
  switching tabs retains its system back path without adding a custom edge
  gesture or horizontal tab swipe. Existing next-launch choices map to the
  corresponding root and stack; content links append to the visible stack,
  while app-only inbox routes activate the exact message kind above a cleared
  message root. Home Screen search, cloud-favorite, and check-in actions retain
  their unique-landing and active-check-in preservation rules. Inactive visible
  roots cancel or defer their network work, account changes
  clear private snapshots without an offscreen reload, and changing the main
  surface revokes active voice or video playback. The Messages tab reuses the
  authoritative active-account reply-plus-mention badge, excludes the separate
  fan reminder, never clears a server count locally, and opens global search
  inside the Messages stack. My retains account management while adding direct
  local-favorite, history, appearance, settings, and About controls; its typed
...
- **`v0.65.0-alpha.8` responsive Home and thread navigation:** Home's followed-forum
  surface removes its duplicate horizontal List inset so cards align with adjacent
  sections. Its stored layout control now produces a compact adaptive grid with a
  dedicated tile presentation or a detailed full-width list instead of two visually
  identical single-column states; large text sizes retain a guarded single-column
  fallback. Thread detail restores the system inline thread title rather than
  replacing it with a forum shortcut. Compact-width and large-text navigation bars
  expose only Share and a unified More menu, while regular-width bars keep the common
  reading and favorite shortcuts. The unified menu retains reading mode, local and
  cloud favorites, page jump, forum navigation, report, and owned-content deletion;
  its report action receives the exact presentation scope and the menu always has a
  concrete item, eliminating the previously tappable but empty ellipsis control.
...
- **`v0.64.0-alpha.5` followed-forum check-in marks:** Home and the complete current-
...
- **`v0.64.0` OLED dark surfaces:** Appearance and dark-surface selection are
  ...
  The opt-in palette uses a black page/content canvas with bounded near-black
  card, floor, control, divider, and high-contrast layers instead of flattening
  every surface to one color. App-owned Lists, thread and nested-reply lazy
  stacks, search and filter bars, composers, settings, and navigation chrome
  participate. System Safari, Web login content, share sheets, keyboards, media
  viewers, semantic warning colors, and media fills remain system-managed. The
...
- **Current alpha:** `v0.65.0-alpha.8` publishes the four-root Home, Explore,
  ...
  `v0.64.0-alpha.6`'s complete
  protobuf image-source fallback for thread summaries, ordinary floors, and
  full nested replies, with one shared media URL authorization policy, and
  `v0.64.0-alpha.5`'s account-bound followed-forum today-check-in marks,
  including monotonic same-day reconciliation, bounded foreground refresh,
  active UTC+8 expiry, and strict forum-identity validation. It retains
  `v0.64.0-alpha.4`'s independent OLED
  dark-surface option and optimized thread and nested-reply render boundaries.
  It retains `v0.64.0-alpha.3`'s account and
  cloud-favorite hardening: an active complete account can explicitly hand off
...
- **`v0.63.0` owner deletion and cloud-favorite updates:** An active full-
  ...
  only-author option. Hot order exposes no exact-position action. Invalid,
...
- **Historical `v0.60.5-alpha.1` release scope:** The complete Discovery section
  ...
  On iOS 18, each floor reports its own visibility through the native scroll
  visibility API; iOS 16 and 17 retain the geometry-preference compatibility
  path. Stable content revisions and cached render plans move content-block
  construction plus plain and linked text projection out of repeated row
  updates. Unchanged post
  and nested-reply plans survive append, prepend, and agreement-context rebuilds,
  ...
  Thread and nested-reply rows avoid persistent
  direct text selection while retaining the existing context-menu selection
  panel. Inline nested-reply previews render at most three non-hidden rows and
  preserve the complete-list action for every overflow or filtered case. Normal
  post and comment pagination updates lookup, visibility, agreement, and scroll
  indexes incrementally without copying or rescanning the full retained list;
  replacements and genuine agreement-context changes still rebuild atomically.
  On iOS 18, only confirmed animated thumbnails subscribe to viewport changes and show
  a poster while offscreen. Across supported OS versions, their display link
  pauses during direct touch tracking. Thread preview decoding is bounded to
  1,320 pixels. Ordinary thumbnails no longer subscribe to download-progress
  events that they do not render; the full-screen image viewer retains its
  progress reporting.
  The personalized feed prefetches near its final three rows and treats every
  nonempty raw page as eligible for continuation because the endpoint has no
  authoritative `has_more`; an empty raw page ends pagination. Each load attempt,
  including automatic prefetch, may cross at most five consecutive nonempty pages
  whose entries all fail mapping before requiring an explicit retry. The optional
  followed-forum filter independently scans at most five mapped-but-filtered
  pages per load attempt, while duplicate-page and retained-item bounds remain in
  force. Explore opens on a credential-free
  personalized thread feed beside the existing hot ranking. A logged-in account
  ...
  and an unknown write outcome locks only that row until a successful refresh or
...
- **`v0.65.0-alpha.2` cloud-favorite author profiles:** Cloud-favorite rows now retain
...
- **Automated checks:** GitHub Actions runs package tests and the complete iOS
  ...
  UID/session-lease race handling, own-following-list mutation, retained-row
  refollow, relationship notification isolation, target-bound interaction permissions,
...
- **App source:** Add [`sidestore-source.json`](https://raw.githubusercontent.com/Minaduki-Shigure/tieba-plus-plus-swift/main/sidestore-source.json)
  ...
  The source currently distributes the verified `v0.65.0-alpha.8` IPA (build 86).
...
- **Search:** Forum, thread, and user search are separated by category. Global
  ...
  same-thread targets are present and valid. `v0.65.0-alpha.2` also highlights the
...
- **Public information:** Forum introductions, statistics, rules, moderator
  ...
  bounded server-provided creator-field labels with a generic verification
  fallback and precise known moderator-role labels. These labels are display
  metadata only and never authorize an account or management action. Profiles
  also include independently paginated public topics and replies; other-user
  following and all follower lists are also public, while the exact active account's own
...
- **Images:** Responsive image groups open in a zoomable gallery with horizontal
  ...
  dynamic long-image zoom ceiling permits readable portrait and landscape
  widths. Static gallery decoding may retain up to a 16,384-pixel long edge only
  when its measured decoded storage remains within 64 MiB.
...
- **Detailed parity:** See [`ROADMAP.md`](ROADMAP.md) for the complete TiebaLite
  ...
  The public `v0.65.0-alpha.8` app-code snapshot has the same 80–82% weighted
