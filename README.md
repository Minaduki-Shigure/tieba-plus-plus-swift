...
| TiebaLite parity | Current `main` and public `v0.65.0-alpha.6`: about 81% overall (estimated range 80–82%, with 18–20% remaining). Anonymous reading and media remain about 91–95% |
| Distribution | The public SideStore/LiveContainer source currently serves `v0.65.0-alpha.6` (build 84) |
...
- **`v0.65.0-alpha.6` list and gallery interaction:** Home's Explore shortcut uses
  the complete visible List row as its action surface while retaining the same
  standard row height as the adjacent hot-topic link. Explore thread thumbnails
  are display-only, removing the ambiguous image Button that could intercept
  title or excerpt taps inside an iOS List cell; image opening remains available
  from thread content and non-Explore summary surfaces. Personalized recommendation
  cards no longer attach a horizontal swipe action or reserve a full-height side
  column for feedback: the row keeps its full width and exposes the account-bound
  `Not interested` action as a visible lower-trailing button. Forum, author, and
  time metadata stay on one truncating line at ordinary text sizes while retaining
  a stacked layout at expanded Dynamic Type sizes. Full-screen image viewers route Close and the
...
- **Current alpha:** `v0.65.0-alpha.6` publishes the four-root Home, Explore,
...
- **App source:** Add [`sidestore-source.json`](https://raw.githubusercontent.com/Minaduki-Shigure/tieba-plus-plus-swift/main/sidestore-source.json)
  ...
  The source currently distributes the verified `v0.65.0-alpha.6` IPA (build 84).
...
- **Detailed parity:** See [`ROADMAP.md`](ROADMAP.md) for the complete TiebaLite
  ...
  The public `v0.65.0-alpha.6` app-code snapshot has the same 80–82% weighted
