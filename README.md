...
| TiebaLite parity | Current `main` source and the public `v0.60.4-alpha.1` app-code snapshot: about 80% of full product scope (estimated range 78–81%, with 19–22% remaining); anonymous reading and media: about 91–95% |
| Distribution | The public SideStore/LiveContainer source currently serves `v0.60.4-alpha.1` (build 67) |

### Release and validation

- **Current alpha:** `v0.60.4-alpha.1` publishes all end-user app-code changes
...
- **`v0.60.4-alpha.1` release scope:** The complete Discovery section appears
  first on Home. On iOS 18, thread reading uses native scroll-target visibility
  instead of evaluating per-floor geometry for every scroll frame; iOS 16 and 17
  retain the prior compatibility path. Stable layout and post indexes preserve
  reading progress and earlier-page anchors without scanning the full retained
  floor set on every scroll update. Native visibility changes are coalesced
  before publishing root view state, while the latest raw position remains
  available for history and prepend restoration. Stable render boundaries keep
  unchanged post and nested-reply bodies out of those root updates. Plain text
  bypasses attributed-string construction, while mentions, links, and media
  retain their interactive paths. Thread and nested-reply rows avoid persistent
  ...
  `v0.60.4-alpha.1` IPA after the tagged commit and artifact passed the release
  checks.
- **Non-user-facing image-creation foundation:** The public `v0.60.4-alpha.1`
...
- **App source:** Add [`sidestore-source.json`](https://raw.githubusercontent.com/Minaduki-Shigure/tieba-plus-plus-swift/main/sidestore-source.json)
  ...
  currently distributes the verified `v0.60.4-alpha.1` IPA (build 67).
...
- **Discovery:** On current `main`, a pageable anonymous personalized feed, an
  ...
  public `v0.60.4-alpha.1` IPA. Recommendation dislike feedback remains disabled.
...
- **Detailed parity:** See [`ROADMAP.md`](ROADMAP.md) for the complete TiebaLite
  ...
  The public `v0.60.4-alpha.1` app-code snapshot has the same estimated scope.
