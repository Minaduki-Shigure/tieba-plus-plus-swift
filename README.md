...
| TiebaLite parity | Current `main` and public `v0.65.0-alpha.7`: about 81% overall (estimated range 80–82%, with 18–20% remaining). Anonymous reading and media remain about 91–95% |
| Distribution | The public SideStore/LiveContainer source currently serves `v0.65.0-alpha.7` (build 85) |
...
- **`v0.65.0-alpha.7` deterministic list interaction:** Home's Explore shortcut
  retains its full standard-height action surface. Explore and forum thread rows
  now route their title, metadata, media, and metrics through disjoint full-width
  thread actions; image loading, success, and retry states cannot replace that
  routing with a competing gallery Button. Thread-summary media defaults to opening
  its thread across list surfaces, while image galleries remain available from
  thread content. Personalized recommendation feedback is an independent 44-point
  trailing metric action that shows only `hand.thumbsdown`, stays disabled in
  flight, and adds neither a footer row nor a swipe gesture. Forum, author, and time
  metadata retain their compact and expanded Dynamic Type layouts. Full-screen
  image viewers route Close and the accessibility escape action directly through
  the presentation owner. In
...
- **Current alpha:** `v0.65.0-alpha.7` publishes the four-root Home, Explore,
...
- **App source:** Add [`sidestore-source.json`](https://raw.githubusercontent.com/Minaduki-Shigure/tieba-plus-plus-swift/main/sidestore-source.json)
  ...
  The source currently distributes the verified `v0.65.0-alpha.7` IPA (build 85).
...
- **Detailed parity:** See [`ROADMAP.md`](ROADMAP.md) for the complete TiebaLite
  ...
  The public `v0.65.0-alpha.7` app-code snapshot has the same 80–82% weighted
